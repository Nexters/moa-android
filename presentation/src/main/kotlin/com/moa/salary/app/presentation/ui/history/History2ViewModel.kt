...
@HiltViewModel
class History2ViewModel @Inject constructor(
    private val moaSideEffectBus: MoaSideEffectBus,
    private val calendarRepository: CalendarRepository,
) : ViewModel() {

    private val _uiState = MutableStateFlow(History2UiState())
    val uiState = _uiState.asStateFlow()

    fun onIntent(intent: History2Intent) {
        when (intent) {
            History2Intent.GetCalendar -> getCalendar(YearMonth.now())
            History2Intent.ClickBack -> clickBack()
            is History2Intent.SetYearMonth -> setYearMonth(intent.yearMonth)
            is History2Intent.ClickDate -> clickDate(intent.date)
            is History2Intent.ClickSchedule -> clickSchedule(intent.schedule)
            is History2Intent.ClickPayday -> clickPayday(intent.day)
        }
    }
